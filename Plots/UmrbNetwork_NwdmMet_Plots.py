...
Description: Creates meteorological plots for all Upper Missouri River Basin Monitoring Network gages.
...
PlotFilename = ImagesDirectory + '%s_Misc2.jpg'
...
CurveProperties =   {
                        ...
                        #    --------------         -----------      ------------    ----------    ----------   ------------    -------------    ----------------    ------------    -----------       ----------------    ----------------    ----------------    ----------------    -----------------           
                            'RelHumidity'    :    [ 'blue',          'Solid',        2,            'None',      'darkgreen',    'Solid',         Constants.FALSE,    'Triangle',     7,                'darkgreen',        'darkgreen',        0,                  0,                  0],
                            'DirWind'        :    [ 'red',           'Solid',        2,            'None',      'darkgreen',    'Solid',         Constants.FALSE,    'Triangle',     7,                'darkgreen',        'darkgreen',        0,                  0,                  0],
                            'SpeedWind'      :    [ 'darkgreen',     'Solid',        2,            'None',      'blue',         'Solid',         Constants.FALSE,    'X',            7,                'blue',             'blue',             0,                  0,                  0],
                            'Irrad'          :    [ 'black',         'Solid',        2,            'None',      'red',          'Solid',         Constants.FALSE,    'Circle',       7,                'red',              'red',              0,                  0,                  0],
                            'SoilMoisture40' :    [ 'purple',        'Solid',        2,            'None',      'gray',         'Solid',         Constants.FALSE,    'Diamond',      7,                'gray',             'gray',             0,                  0,                  0],
                            'Other'          :    [ 'darkred',       'Solid',        2,            'None',      'darkred',      'Solid',         Constants.FALSE,    'Square',       7,                'darkred',          'darkred',          0,                  0,                  0],
                    }
...
try :
    ...
    for location in Locations :
        try :
            ...
            # Retrieve time series from database
            # Relative Humidity data
            if (RelativeHumidityAve5minRawMadis % location) in PathnameList :
                try : RelHumidityTsc = CwmsDb.read(RelativeHumidityAve5minRawMadis % location).getData()
                except : RelHumidityTsc = CwmsDb.get(RelativeHumidityAve5minRawMadis % location)
            else :
                # Create a blank time series so the plot can be created
                RelHumidityTsc = createBlankTimeSeries(debug, '%s.%%-RelativeHumidity.Ave.5Minutes.5Minutes.No Available Data' % location, '%', StartTw, EndTwStr)
            # Wind Direction data
            if (DirWindAve5minRawMadis % location) in PathnameList :
                try : DirWindTsc = CwmsDb.read(DirWindAve5minRawMadis % location).getData()
                except : DirWindTsc = CwmsDb.get(DirWindAve5minRawMadis % location)
            else :
                # Create a blank time series so the plot can be created
                DirWindTsc = createBlankTimeSeries(debug, '%s.Dir-Wind.Ave.5Minutes.5Minutes.No Available Data' % location, 'deg', StartTw, EndTwStr)
            # Wind Speed data
            if (SpeedWindAve5minRawMadis % location) in PathnameList :
                try : SpeedWindTsc = CwmsDb.read(SpeedWindAve5minRawMadis % location).getData()
                except : SpeedWindTsc = CwmsDb.get(SpeedWindAve5minRawMadis % location)
            else :
                # Create a blank time series so the plot can be created
                SpeedWindTsc = createBlankTimeSeries(debug, '%s.Speed-Wind.Ave.5Minutes.5Minutes.No Available Data' % location, 'mph', StartTw, EndTwStr)
            # Irrad data
            if (IrradAve5minRawMadis % location) in PathnameList :
                try : IrradTsc = CwmsDb.read(IrradAve5minRawMadis % location).getData()
                except : IrradTsc = CwmsDb.get(IrradAve5minRawMadis % location)
            else :
                # Create a blank time series so the plot can be created
                IrradTsc = createBlankTimeSeries(debug, '%s.Irrad.Total.5Minutes.5Minutes.No Available Data' % location, 'langley/min', StartTw, EndTwStr)
            ...
            # Create viewports
            Viewport1 = layout.addViewport()
            Viewport2 = layout.addViewport()
            Viewport3 = layout.addViewport()
            Viewport4 = layout.addViewport()
            ...
            #                           -----------------     ------------         --------    ----------------
            ViewportLayoutsInfo = [ [   Viewport1,            RelHumidityTsc,      'Y1',       'RelHumidity'],
                                    [   Viewport2,            DirWindTsc,          'Y1',       'DirWind'],
                                    [   Viewport3,            SpeedWindTsc,        'Y1',       'SpeedWind'],
                                    [   Viewport4,            IrradTsc,            'Y1',       'Irrad'],
                                    ]
