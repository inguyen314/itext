...
PlotFilename = ImagesDirectory + '%sp.jpg'

# List of locations to be included on plot
Locations = [   'BLNE', 'PTNE', 'PA11', 'PA16', 'PA18', 'PA20', 'SC02', 'SC04', 'SC08', 'SC09', 'SC10', 'SC12', 'SC13', 'SC14', 'SC17', 'SC18',
                'PIST', 'BOHA', 'COLD', 'COTT', 'CHCR', 'BECR', 'CHFI'
                ]

# Dictionary of data
LocationProperties = {  'BLNE'              :    {  'PlotTitle'         :   'Missouri River at Blair, NE\nFlood Stage = 26.5 ft',
                                                    },
...
CurveProperties =   {
                        ...
                        #    --------------         -----------      ------------    ----------    ----------   ------------    -------------    ----------------    ------------    -----------       ----------------    ----------------    ----------------    ----------------    -----------------           
                            ...
                            'Precip'         :    [ 'blue',          'Solid',        2,            'None',      'darkred',      'Solid',         Constants.FALSE,    'Square',       7,                'darkred',          'darkred',          0,                  0,                  0],
                    }
    ...
    for location in Locations :
        try :
            ...
            if (PrecipInstHourBestNwo % location) in PathnameList :
                if location == 'PIST' :
                    try : PrecipTsc = CwmsDb.read(PrecipInstHourBestNwo % 'PIST-Office').getData()
                    except : PrecipTsc = CwmsDb.get(PrecipInstHourBestNwo % 'PIST-Office')
                else :
                    try : PrecipTsc = CwmsDb.read(PrecipInstHourBestNwo % location).getData()
                    except : PrecipTsc = CwmsDb.get(PrecipInstHourBestNwo % location)
            elif (PrecipInstHourBestNwdm % location) in PathnameList :
                try : PrecipTsc = CwmsDb.read(PrecipInstHourBestNwdm % location).getData()
                except : PrecipTsc = CwmsDb.get(PrecipInstHourBestNwdm % location)
            else :
                # Create a blank time series so the plot can be created
                PrecipTsc = createBlankTimeSeries(debug, '%s.Precip.Inst.1Hour.0.No Available Data' % location, 'in', StartTw, EndTwStr)
            # Stage/Elev data
            if (ElevInstHourBestNwo % location) in PathnameList :
                ...
                # Create a blank time series so the plot can be created
                ElevTsc = createBlankTimeSeries(debug, '%s.Elev.Inst.1Hour.0.No Available Data' % location, 'ft', StartTw, EndTwStr)
