...
Description: Creates elevation, inflow, and outflow with zone markers plots for the mainstem reservoirs in the Missouri River Basin.
...
global ElevInstHourBestMrbwm, FlowOutAveDayBestMrbwm, FlowInAveDayBestMrbwm, TopOfConsZone, TopOfMultiZone, TopOfExclZone
...
MarkerProperties =  {
                        ...
                        #    -----------------------          -----------------------    -------------        --------  -----------    -------------------------------------      ------------       ------------   ---------------    ------------    ------------    ----------     ----------   -----------        
                            'TopOfConsZone'             :   [ True,                      TopOfConsZone,       None,     None,          'Base of Annual Flood Control Zone',       'Arial,Plain,12',  'Y1',          'left',            'above',        'black',        'Dash Dot',    'black',     1],         
                            'TopOfMultiZone'            :   [ True,                      TopOfMultiZone,      None,     None,          'Base of Exclusive Flood Control Zone',    'Arial,Plain,12',  'Y1',          'left',            'above',        'darkred',      'Dash Dot',    'darkred',   1],        
                            'TopOfExclZone'             :   [ True,                      TopOfExclZone,       None,     None,          'Top of Exclusive Flood Control Zone',     'Arial,Plain,12',  'Y1',          'left',            'above',        'darkcyan',     'Dash Dot',    'darkcyan',  1],        
                    }
    ...
    for location in Locations :
        try :
            ...
            if (FlowOutAveDayBestMrbwm % location) in PathnameList :
                try : 
                    FlowOutHm = CwmsDb.read(FlowOutAveDayBestMrbwm % location)
                    # Convert values to kcfs
                    FlowOutHm = FlowOutHm.divide(1000)
                    FlowOutHm.setUnits('kcfs')
                    
                    FlowOutTsc = FlowOutHm.getData()
                except :
                    FlowOutTsc = CwmsDb.get(FlowOutAveDayBestMrbwm % location)
            else :
                # Create a blank time series so the plot can be created
                FlowOutTsc = createBlankTimeSeries(debug, '%s.Flow-In.Ave.~1Day.1Day.Best-MRBWM' % location, 'cfs', StartTw, EndTwStr)
            if (FlowInAveDayBestMrbwm % location) in PathnameList :
                try :
                    FlowInHm = CwmsDb.read(FlowInAveDayBestMrbwm % location)
                    # Convert values to kcfs
                    FlowInHm = FlowInHm.divide(1000)
                    FlowInHm.setUnits('kcfs')
                    
                    FlowInTsc = FlowInHm.getData()
                except : 
                    FlowInTsc = CwmsDb.get(FlowInAveDayBestMrbwm % location)
            else :
                # Create a blank time series so the plot can be created
            ...
            BottomViewportLayout = layout.addViewport()
            ...
            ViewportLayoutsInfo = [ [   TopViewportLayout,    ElevTsc,        'Y1',       'Elev'],
                                    [   BottomViewportLayout, FlowInTsc,      'Y1',       'Inflow'],
                                    [   BottomViewportLayout, FlowOutTsc,     'Y1',       'Flow']
                                        ]
