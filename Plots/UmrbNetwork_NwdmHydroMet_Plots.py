...
PlotFilename = ImagesDirectory + '%s_Misc1.jpg'
...
CurveProperties =   {
                        ...
                        #    --------------         -----------      ------------    ----------    ----------   ------------    -------------    ----------------    ------------    -----------       ----------------    ----------------    ----------------    ----------------    -----------------           
                            'SnowDepth'      :    [ 'blue',          'Solid',        2,            'None',      'darkgreen',    'Solid',         Constants.FALSE,    'Triangle',     7,                'darkgreen',        'darkgreen',        0,                  0,                  0],
                            'Precip'         :    [ 'red',           'Solid',        2,            'None',      'darkgreen',    'Solid',         Constants.FALSE,    'Triangle',     7,                'darkgreen',        'darkgreen',        0,                  0,                  0],
                            'TempAir'        :    [ 'darkgreen',     'Solid',        2,            'None',      'blue',         'Solid',         Constants.FALSE,    'X',            7,                'blue',             'blue',             0,                  0,                  0],
    ...
    for location in Locations :
        try :
            ...
            try : 
                ...
                PlotTitle = 'UMRB Monitoring Network - %s\nHydrometeorological Data' % PublicLocation
            ...
            # Snow Depth data
            if (DepthSnowInstHourRawMadis % location) in PathnameList :
                try : SnowDepthTsc = CwmsDb.read(DepthSnowInstHourRawMadis % location).getData()
                except : SnowDepthTsc = CwmsDb.get(DepthSnowInstHourRawMadis % location)
            else :
                # Create a blank time series so the plot can be created
                SnowDepthTsc = createBlankTimeSeries(debug, '%s.Depth-Snow.Inst.1Hour.0.No Available Data' % location, 'in', StartTw, EndTwStr)
            # Precipitation data
            if (PrecipTotal5minRawMadis % location) in PathnameList :
                try : PrecipTsc = CwmsDb.read(PrecipTotal5minRawMadis % location).getData()
                except : PrecipTsc = CwmsDb.get(PrecipTotal5minRawMadis % location)
            else :
                # Create a blank time series so the plot can be created
                PrecipTsc = createBlankTimeSeries(debug, '%s.Precip.Total.5Minutes.5Minutes.No Available Data' % location, 'in', StartTw, EndTwStr)
            # Air Temperature data
            if (TempAve5minRawMadis % location) in PathnameList :
                try : TempAirTsc = CwmsDb.read(TempAve5minRawMadis % location).getData()
                except : TempAirTsc = CwmsDb.get(TempAve5minRawMadis % location)
            else :
                # Create a blank time series so the plot can be created
                TempAirTsc = createBlankTimeSeries(debug, '%s.Temp-Air.Ave.5Minutes.5Minutes.No Available Data' % location, 'F', StartTw, EndTwStr)
            ...
            # Create viewports
            TopViewport = layout.addViewport()
            MiddleViewport = layout.addViewport()
            BottmViewport = layout.addViewport()
            ...
            #                           -----------------     ------------         --------    ----------------
            ViewportLayoutsInfo = [ [   TopViewport,          SnowDepthTsc,        'Y1',       'SnowDepth'],
                                    [   MiddleViewport,       PrecipTsc,           'Y1',       'Precip'],
                                    [   BottmViewport,        TempAirTsc,          'Y1',       'TempAir'],
                                    ]
