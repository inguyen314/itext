...
Description: Creates flow plots for specified NWK projects. Various projects all plotted on the same viewport.
...
LocationProperties = {  # MoRiv Stations
                        'kanproj'   :   {   'Locations'     :   ['CLIN', 'MILD', 'PERY', 'TUCR'],
                                            'PlotTitle'     :   'Kansas River Project Discharges',
                                            },
                }
        ...
        try :
            ...
            for x in range(len(Locations)) :
                ...
                if (FlowOutAveDayBestNwk % location) in PathnameList :
                    try : Tsc = CwmsDb.read(FlowOutAveDayBestNwk % location).getData()
                    except : Tsc = CwmsDb.get(FlowOutAveDayBestNwk % location)
                else :
                    # Create a blank time series so the plot can be created
                    Tsc = createBlankTimeSeries(debug, '%s.Flow-Out.Inst.1Hour.0.No Available Data' % location, 'cfs', StartTw, EndTwStr)
