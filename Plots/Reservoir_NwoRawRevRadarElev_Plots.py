...
PlotFilename = ImagesDirectory + '%sr.jpg'
...
# Dictionary of data
LocationProperties = {  'COLD'              :   {   'PlotTitle'         :   'Cold Brook Dam on Cold Brook, Hot Springs, SD',
                                                    },
...
CurveProperties =   {
                        ...
                        #    --------------         -----------      ------------    ----------    ----------   ------------    -------------    ----------------    ------------    -----------       ----------------    ----------------    ----------------    ----------------    -----------------           
                            ...
                            'RawElevRadar'   :    [ 'black',         'Solid',        1,            'None',      'black',        'Solid',         Constants.FALSE,    'X',            7,                'blue',             'blue',             0,                  0,                  0],
                    }
...
try :
    ...
    for location in Locations :
        try :
            ...
            if (ElevRadarInst15minRawLrgs % location) in PathnameList :
                try : RadarTsc = CwmsDb.read(ElevRadarInst15minRawLrgs % location).getData()
                except : RadarTsc = CwmsDb.get(ElevRadarInst15minRawLrgs % location)
            else :
                # Create a blank time series so the plot can be created
                RadarTsc = createBlankTimeSeries(debug, '%s.Elev.Inst.1Hour.0.No Available Data' % location, 'ft', StartTw, EndTwStr)
            ...
            ViewportLayoutsInfo = [ [   TopViewportLayout,    RevTsc,         'Y1',       'RevElev'],
                                    ...
                                    [   TopViewportLayout,    RadarTsc,       'Y1',       'RawElevRadar']
                                        ]
