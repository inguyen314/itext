...
#-----------------------------------------#
def plotData (dbi, stage, flow, titlename, width, height, jpgname, quality) :
...
# define the plot, add data, etc.		
		...
		try :
			plot.addData(stageData)
	                plot.addData(flowData)
			
			
		except :
			return
#                plot.addData(toLocalTime(stageData))
#E                plot.addData(toLocalTime(flowData))
#                stageContainer = stageData.getData()
#                flowContainer = flowData.getData()
                        # chop quality to work-around missing interpolation bug - deleted ice flow at BIS
#                stageContainer.quality = None
#                flowContainer.quality = None
#                plot.addData(toLocalTime(stageContainer))
                #plot.addData(toLocalTime(flowContainer))
...
#
dbiName     = "foo"
iniFilename = "/cwms/g7cwmspd/plotFiles/plotDataMRRnonfui-qgh.ini"
...
while 1 :
	...
	plotData(dbi, iniStage, iniFlow, iniTitle, plotWidth, plotHeight, iniFname, plotQuality)
