...
#-----------------------------------------#
def plotData (dbi, wtemp, titlename, width, height, jpgname, quality) :

	if debugOutput :
		print "Water Temp 	= %s" % wtemp
...
#
# customize the first graph on the plot (first line in the ini data file) ... water temp ... viewport0		
		viewport0=plot.getViewport(0)
# set yaxis scale - added for Oahe negative values
		yaxis=viewport0.getAxis("Y1")
		ymin=yaxis.getViewMin()
		ymax=yaxis.getViewMax()
		if ymin < 0 and Elev=="OAHE-Oahe Dam-Missouri.Elev.Inst.1Hour.0.mrrppcs-rev": 
			ymin = 1583.
			ymax = 1593.
			yaxis.setScaleLimits(ymin,ymax)
...
#
dbiName     = "foo"
iniFilename = "/cwms/g7cwmspd/plotFiles/plotDataMRRWaterTemp1.ini"
...
while 1 :
	try :	
		iniWatertemp 	= iniFile.readline().strip()
	...
	plotData(dbi, iniWatertemp, iniTitle, plotWidth, plotHeight, iniFname, plotQuality)
