...
#-----------------------------------------#
def plotData (dbi, obsFlow, measFlow, nwsFlow, usgsFlow, fuiD1Flow, fuiFlow, titlename, width, height, jpgname, quality) :

	if debugOutput :
		print "Observed Flow	= %s" % obsFlow
		print "USGS Measure	= %s" % measFlow
		print "NWS Forecast	= %s" % nwsFlow
		print "USGS Flow	= %s" % usgsFlow
		print "FUI Yesterday	= %s" % fuiD1Flow
...
# read the data from the CWMS Oracle database via the DBI from the ini data file
		...
		fuiD1FlowData 	= dbi.get(fuiD1Flow)
...
#
		try :
			...
			nwsFlowDC = toLocalTime(nwsFlowData)
			usgsFlowDC = toLocalTime(usgsFlowData)
			fuiD1FlowDC = toLocalTime(fuiD1FlowData)
			fuiFlowDC = toLocalTime(fuiFlowData)
			try :
				measFlowDC = toLocalTime(measFlowData)
			except :
...
# define the plot layout
		...
		view.addCurve("Y1",nwsFlowDC)
		view.addCurve("Y1",usgsFlowDC)
		view.addCurve("Y1",fuiD1FlowDC)
		...
		plot.setLegendLabelText(nwsFlowDC,"NWS Forecasted Flow")
		plot.setLegendLabelText(usgsFlowDC,"USGS Calculated Flow")
		plot.setLegendLabelText(fuiD1FlowDC,"COE Yesterday's Forecast")
...
#
# set line color, style, width for time series ..... NWS forecasted flow
                lineColor3=plot.getCurve(nwsFlowData)
                lineColor3.setLineColor("magenta")
                lineColor3.setLineStyle("Dash")
                lineColor3.setLineWidth( 1. )
#
# set line color, style, width for time series ..... USGS calculated flow
                lineColor4=plot.getCurve(usgsFlowData)
                lineColor4.setLineColor("green")
                lineColor4.setLineStyle("Solid")
                lineColor4.setLineWidth( 1. )
#
# set line color, style, width for time series ..... COE FUI yesterday's forecasted flow
                lineColor5=plot.getCurve(fuiD1FlowData)
...
# set line color, style, width for time series ..... COE FUI forecasted flow
                lineColor6=plot.getCurve(fuiFlowData)
                lineColor6.setLineColor("blue")
                lineColor6.setLineStyle("Dash")
                lineColor6.setLineWidth( 1. ) 
#
...
#
dbiName     = "foo"
iniFilename = "/cwms/g7cwmspd/plotFiles/plotDataMRRfcastq.ini"
timeSpan    = 35 * 24 * 60 # minutes
...
while 1 :
	try :	
		...
		iniFuiD1Flow 	= iniFile.readline().strip()
	...
	plotData(dbi, iniObsFlow, iniMeasFlow, iniNwsFlow, iniUsgsFlow, iniFuiD1Flow, iniFuiFlow, iniTitle, plotWidth, plotHeight, iniFname, plotQuality)
