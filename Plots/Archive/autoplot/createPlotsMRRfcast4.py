...
#-----------------------------------------#
def plotData (dbi, obsFlow, fuiFlow, obsStage, nwsStage, titlename, width, height, jpgname, quality) :
...
# read the data from the CWMS Oracle database via the DBI from the ini data file
		...
		#fui values are now 4 per day in a .1Hour time series
		#this estimates the other values so that the line actually plots
#		fuiFlowData = fuiFlowData.estimateForMissingValues(6) 
...
#
# set line color, style, width for time series - NWK Forecasted Stage
		if numlines <> 3:
                	lineColor4=plot.getCurve(nwsStageData)
                	lineColor4.setLineColor("orange")
...
#
dbiName     = "foo"
iniFilename = "/cwms/g7cwmspd/plotFiles/plotDataMRRfcast4.ini"
...
while 1 :
	...
	plotData(dbi, iniObsFlow, iniFuiFlow, iniObsStage, iniNwsStage, iniTitle, plotWidth, plotHeight, iniFname, plotQuality)
