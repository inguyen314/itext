...
#-----------------------------------------#
def plotData (dbi, obsFlow, fuiFlow, obsStage, nwsStage, nwsFlow, titlename, width, height, jpgname, quality) :

	if debugOutput :
		print "Observed Flow		= %s" % obsFlow
		print "FUI Forecasted Flow	= %s" % fuiFlow
		print "Observed Stage		= %s" % obsStage
		print "NWS Forecasted Stage	= %s" % nwsStage
		print "NWS Forecasted Flow	= %s" % nwsFlow
...
# read the data from the CWMS Oracle database via the DBI from the ini data file
		...
		obsStageData = dbi.get(obsStage)
		nwsStageData = dbi.get(nwsStage)
		nwsFlowData = dbi.get(nwsFlow)
# define the plot, add data, etc.		
		...
		try :
			plot.addData(toLocalTime(obsFlowData.getData()))
			plot.addData(toLocalTime(fuiFlowData.getData()))
			plot.addData(toLocalTime(obsStageData.getData()))
			try:
				plot.addData(toLocalTime(nwsStageData.getData()))
				plot.addData(toLocalTime(nwsFlowData.getData()))
			except:
				print "missing nws stage and flow"
				numlines = 3
		except :
			return
#		try :
...
#			continue
		plot.showPlot()
#
# set line color, style, width for time series - observed flow
                lineColor1=plot.getCurve(obsFlowData)
                lineColor1.setLineColor("red")
                lineColor1.setLineStyle("Solid")
                lineColor1.setLineWidth( 1. )
#
# set line color, style, width for time series - FUI forecasted flow
                lineColor2=plot.getCurve(fuiFlowData)
                lineColor2.setLineColor("blue")
                lineColor2.setLineStyle("Dash")
                lineColor2.setLineWidth( 1. )
#
# set line color, style, width for time series - observed Stage
                lineColor3=plot.getCurve(obsStageData)
                lineColor3.setLineColor("black")
                lineColor3.setLineStyle("Solid")
                lineColor3.setLineWidth( 1. )
#
# set line color, style, width for time series - NWK Forecasted Stage and Flow
		if numlines <> 3:
                	lineColor4=plot.getCurve(nwsStageData)
                	lineColor4.setLineColor("darkcyan")
                	lineColor4.setLineStyle("Dash")
                	lineColor4.setLineWidth( 1. )
#
                	lineColor5=plot.getCurve(nwsFlowData)
                	lineColor5.setLineColor("purple")
                	lineColor5.setLineStyle("Dash")
                	lineColor5.setLineWidth( 1. )
#
# customize the first graph on the plot ... flow ... viewport0		
...
#
# customize the second graph on the plot ... stage ... viewport1		
...
#
dbiName     = "foo"
iniFilename = "/cwms/g7cwmspd/plotFiles/plotDataMRRfcast5.ini"
...
while 1 :
	try :	
		...
		iniObsStage 	= iniFile.readline().strip()
		iniNwsStage 	= iniFile.readline().strip()
		iniNwsFlow 	= iniFile.readline().strip()
	...
	plotData(dbi, iniObsFlow, iniFuiFlow, iniObsStage, iniNwsStage, iniNwsFlow, iniTitle, plotWidth, plotHeight, iniFname, plotQuality)
