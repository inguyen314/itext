...
#-----------------------------------------#
def plotData (dbi, elevation, inflow, release, basefc, basenorm, basemax, titlename, width, height, jpgname, quality) :

	#if debugOutput :
	print "Elevation	= %s" % elevation
	print "Inflow 		= %s" % inflow
	print "Total Release	= %s" % release
...
# read the data from the CWMS Oracle database via the DBI from the ini data file
	elevationData = dbi.get(elevation)
	inflowData = dbi.get(inflow)
	inflowData = TimeSeriesMath(inflowData)
	inflowDataKcfs = inflowData.divide(1000.)
	inflowDataKcfs.setUnits("kcfs")
	inflowDataKcfs2 = inflowDataKcfs.getData()
	releaseData = dbi.get(release)
	releaseData = TimeSeriesMath(releaseData)
	releaseDataKcfs = releaseData.divide(1000.)
	releaseDataKcfs.setUnits("kcfs")
	releaseDataKcfs2 = releaseDataKcfs.getData()
...
# define the plot, add data, etc.		
	...
	try:
                plot.addData(elevationData)
                plot.addData(inflowDataKcfs2)
                plot.addData(releaseDataKcfs2)
	...
	plot.showPlot()
#
# set line color, style, width for time series elevation
        lineColor1=plot.getCurve(elevationData)
...
# set line color, style, width for time series inflow
        lineColor2=plot.getCurve(inflowDataKcfs2)
...
# set line color, style, width for time series release
        lineColor3=plot.getCurve(releaseDataKcfs2)
        lineColor3.setLineColor("red")
        lineColor3.setLineStyle("Solid")
        lineColor3.setLineWidth( 2. )
#
# customize the first graph on the plot (first line in the ini data file) ... elevation ... viewport0		
...
#
dbiName     = "foo"
iniFilename = "/cwms/g7cwmspd/plotFiles/plotDataMRRmainstem-res2.ini"
...
while 1 :
	try :	
		iniElevation 	= iniFile.readline().strip()
	...
	plotData(dbi, iniElevation, iniInflow, iniRelease, iniBaseFC, iniBaseNorm, iniBaseMax, iniTitle, plotWidth, plotHeight, iniFname, plotQuality)
