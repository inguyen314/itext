...
Description: Creates observed vs forecast reservoir release plots for KRFT reservoirs.
...
if OsName[ : 7] == 'windows' : 
    # PC pathnames
    CronjobsDirectory = "C:\\local_software\\scripts\\NWK\\cronjobs\\"
...
Locations = [   # KRFT Stations
                 'MILD', 'TUCR', 'PERY', 'CLIN']
#Locations = [   # KRFT River Stations
#                'MEKS', 'TSTK', 'NWCK', 'GLNK', 'NLSK', 'EPKS', 'MILD', 'FRI', 'TUCR', 'WGKS', 'TPAK', 'PERY', 'LEKS', 'CLIN', 'LWRK', 'DESO'
#                ]
# Dictionary of data
LocationProperties = {}
    ...
    for location in Locations :
        try :
            ...
            if (FlowOutAveDayBestNwk % location) in PathnameList :
                try : ObsFlowTsc = CwmsDb.read(FlowOutAveDayBestNwk % location).getData()
                except : ObsFlowTsc = CwmsDb.get(FlowOutAveDayBestNwk % location)                
            ...
            if (FlowOutInst6HourFcstNwkKrft % location) in PathnameList :
                try : FcstFlowTsc = CwmsDb.read(FlowOutInst6HourFcstNwkKrft % location, StartFcstTwStr, EndFcstTwStr).getData()
                except : FcstFlowTsc = CwmsDb.get(FlowOutInst6HourFcstNwkKrft % location, StartFcstTwStr, EndFcstTwStr)
            elif (FlowInst6HourFcstNwkKrft % location) in PathnameList :
                try : FcstFlowTsc = CwmsDb.read(FlowInst6HourFcstNwkKrft % location, StartFcstTwStr, EndFcstTwStr).getData()
                except : FcstFlowTsc = CwmsDb.get(FlowInst6HourFcstNwkKrft % location, StartFcstTwStr, EndFcstTwStr)
            else :
                # Create a blank time series so the plot can be created
