...
Description: Creates soil temperature plots for all Upper Missouri River Basin Monitoring Network gages.
...
PlotFilename = ImagesDirectory + '%s_ST.jpg'
...
CurveProperties =   {
                        ...
                        #    --------------            -----------      ------------    ----------    ----------   ------------    -------------    ----------------    ------------    -----------       ----------------    ----------------    ----------------    ----------------    -----------------           
                            'SoilTemperature02' :    [ 'blue',          'Solid',        2,            'None',      'darkgreen',    'Solid',         Constants.FALSE,    'Triangle',     7,                'darkgreen',        'darkgreen',        0,                  0,                  0],
                            'SoilTemperature04' :    [ 'red',           'Solid',        2,            'None',      'darkgreen',    'Solid',         Constants.FALSE,    'Triangle',     7,                'darkgreen',        'darkgreen',        0,                  0,                  0],
                            'SoilTemperature08' :    [ 'darkgreen',     'Solid',        2,            'None',      'blue',         'Solid',         Constants.FALSE,    'X',            7,                'blue',             'blue',             0,                  0,                  0],
                            'SoilTemperature20' :    [ 'black',         'Solid',        2,            'None',      'red',          'Solid',         Constants.FALSE,    'Circle',       7,                'red',              'red',              0,                  0,                  0],
                            'SoilTemperature40' :    [ 'purple',        'Solid',        2,            'None',      'gray',         'Solid',         Constants.FALSE,    'Diamond',      7,                'gray',             'gray',             0,                  0,                  0],
    ...
    for location in Locations :
        try :
            ...
            for depth in [2, 4, 8, 20, 40] : 
                # Soil Moisture data
                if (SoilTempInstHourRawMadis % (location, depth)) in PathnameList :
                    try : SoilTemperatureTsc = CwmsDb.read(SoilTempInstHourRawMadis % (location, depth)).getData()
                    except : SoilTemperatureTsc = CwmsDb.get(SoilTempInstHourRawMadis % (location, depth))
                else :
                    # Create a blank time series so the plot can be created
                    SoilTemperatureTsc = createBlankTimeSeries(debug, '%s-D%02din.%%-SoilTemperature.Inst.1Hour.0.No Available Data' % (location, depth), '%', StartTw, EndTwStr)
                TimeSeriesContainerList.append(SoilTemperatureTsc)
            ...
            #                           -----------------     ------------                  --------    ----------------
            ViewportLayoutsInfo = [ [   ViewportList[0],      TimeSeriesContainerList[0],   'Y1',       'SoilTemperature02'],
                                    [   ViewportList[1],      TimeSeriesContainerList[1],   'Y1',       'SoilTemperature04'],
                                    [   ViewportList[2],      TimeSeriesContainerList[2],   'Y1',       'SoilTemperature08'],
                                    [   ViewportList[3],      TimeSeriesContainerList[3],   'Y1',       'SoilTemperature20'],
                                    [   ViewportList[4],      TimeSeriesContainerList[4],   'Y1',       'SoilTemperature40'],
                                    ]
