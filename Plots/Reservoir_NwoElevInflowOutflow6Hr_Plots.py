...
PlotFilename = ImagesDirectory + '%sin.jpg'
...
CurveProperties =   {
                        ...
                        #    --------------         -----------      ------------    ----------    ----------   ------------    -------------    ----------------    ------------      -----------       ----------------    ----------------    ----------------    ----------------    -----------------           
                            'Elev'           :    [ 'darkgreen',     'Solid',        2,            'None',      'darkgreen',    'Solid',         Constants.FALSE,    'Open Triangle',  7,                'darkgreen',        'darkgreen',        0,                  0,                  0],
                            ...
                            'Inflow'         :    [ 'red',           'Solid',        2,            'None',      'red',          'Solid',         Constants.FALSE,    'Open Circle',    7,                'red',              'red',              0,                  0,                  0],
...
try :
    ...
    for location in Locations :
        try :
            ...
            # Flow-In data
            if (FlowInAve6HourBestNwo % location) in PathnameList :
                try : FlowInTsc = CwmsDb.read(FlowInAve6HourBestNwo % location).getData()
                except : FlowInTsc = CwmsDb.get(FlowInAve6HourBestNwo % location)
            else :
                # Create a blank time series so the plot can be created
                FlowInTsc = createBlankTimeSeries(debug, '%s.Flow-In.Ave.1Hour.6Hours.Best-NWO' % location, 'cfs', StartTw, EndTwStr)
            # Flow-Out data
            if (FlowOutAve6HourBestNwo % location) in PathnameList :
                try : FlowOutTsc = CwmsDb.read(FlowOutAve6HourBestNwo % location).getData()
                except : FlowOutTsc = CwmsDb.get(FlowOutAve6HourBestNwo % location)
            else :
                # Create a blank time series so the plot can be created
                FlowOutTsc = createBlankTimeSeries(debug, '%s.Flow-Out.Ave.1Hour.6Hours.Best-NWO' % location, 'cfs', StartTw, EndTwStr)
            ...
            for key in MarkerProperties.keys() :
                if MarkerProperties[key][0] :
                    ...
                    try : ZoneValue = retrieveLocationLevel(debug, conn, CwmsDb, ZoneFullName)
