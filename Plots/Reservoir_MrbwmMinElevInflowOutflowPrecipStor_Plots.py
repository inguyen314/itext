...
PlotFilename = ImagesDirectory + '%smin.jpg'
...
MarkerProperties =  {
                        ...
                        #    -----------------------          -----------------------    -------------        --------  -----------    -------------------------------------      ------------       ------------   ---------------    ------------    ------------    ----------     ----------   -----------        
                            'MinElev'                   :   [ True,                      None,                None,     None,          'Record Minimum Pool',                     'Arial,Plain,12',  'Y1',          'left',            'above',        'darkgreen',    'Dash Dot',    'darkgreen', 1],         
...
try :
    ...
    HistoricStartTwStr = '01Jan1967 2400'
    ...
    for location in Locations :
        try :
            ...
            if (ElevInstHourBestMrbwm % location) in PathnameList :
                try : ElevTsc = CwmsDb.read(ElevInstHourBestMrbwm % location).getData()
                except : ElevTsc = CwmsDb.get(ElevInstHourBestMrbwm % location)
            else :
                # Create a blank time series so the plot can be created
                ElevTsc = createBlankTimeSeries(debug, '%s.Elev.Inst.1Hour.0.Best-MRBWM' % location, 'ft', StartTw, EndTwStr)
            # Flow-In data
            if (FlowInAveDayBestMrbwm % location) in PathnameList :
                try : FlowInTsc = CwmsDb.read(FlowInAveDayBestMrbwm % location).getData()
                except : FlowInTsc = CwmsDb.get(FlowInAveDayBestMrbwm % location)
            else :
                # Create a blank time series so the plot can be created
                FlowInTsc = createBlankTimeSeries(debug, '%s.Flow-In.Ave.1Day.1Day.Best-MRBWM' % location, 'cfs', StartTw, EndTwStr)
            # Flow-Out data
            if (FlowOutAveDayBestMrbwm % location) in PathnameList :
                try : FlowOutTsc = CwmsDb.read(FlowOutAveDayBestMrbwm % location).getData()
                except : FlowOutTsc = CwmsDb.get(FlowOutAveDayBestMrbwm % location)
            else :
                # Create a blank time series so the plot can be created
                FlowOutTsc = createBlankTimeSeries(debug, '%s.Flow-Out.Ave.1Day.1Day.Best-MRBWM' % location, 'cfs', StartTw, EndTwStr)
             # Storage data
            if (StorInstDayBestMrbwm % location) in PathnameList :
                try : StorTsc = CwmsDb.read(StorInstDayBestMrbwm % location).getData()
                except : StorTsc = CwmsDb.get(StorInstDayBestMrbwm % location)
            else :
                # Create a blank time series so the plot can be created
                StorTsc = createBlankTimeSeries(debug, '%s.Stor.Inst.1Day.0.Best-MRBWM' % location, 'acre-ft', StartTw, EndTwStr)
             # Precip data
            if (PrecipTotalDayBestMrbwm % location) in PathnameList :
                try : PrecipTsc = CwmsDb.read(PrecipTotalDayBestMrbwm % location).getData()
                except : PrecipTsc = CwmsDb.get(PrecipTotalDayBestMrbwm % location)
            else :
                # Create a blank time series so the plot can be created
                PrecipTsc = createBlankTimeSeries(debug, '%s.Precip.Total.1Day.1Day.Best-MRBWM' % location, 'in', StartTw, EndTwStr)
            ...
            for key in MarkerProperties.keys() :
                if MarkerProperties[key][0] :
                    ...
                    ElevValues = CwmsDb.get(ElevInstHourBestMrbwm % location, HistoricStartTwStr, EndTwStr).values
                    ElevValues = [x if x != Constants.UNDEFINED else 9e6 for x in ElevValues]
                    HistoricMinElev = round(min(ElevValues), 1)
                    MarkerProperties[key][2] = HistoricMinElev
