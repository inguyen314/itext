...
BulletinPropertiesPathname = "D:\\Projects\\HEC_DSSVue_Scripts\\MRBWM\\NWO-CWMS2_Scripts\\Bulletins\\NWO_Levee\\NWO_Levee_Bulletin_Properties.txt"
'''
...
BulletinPropertiesPathname = BulletinScriptDirectory + 'NWO_Reservoir_Bulletin_Properties.txt'
    ...
    for project in DataBlockDict['DataBlocks'][TableDataName]['ProjectList'] :
        ...
        for data in GageData :
            ...
            # Peak Forecasted Stage
                ...
                try :
                    if project in ['BVNE', 'WSN'] :
                        PeakFcstStage = Null
                        # Create a formatted string that will be added to the table
                        CellData = Phrase(Chunk(Null, TextFont))
                    else :
                        StageGageTsc = CwmsDb.read(DataBlockDict['DataBlocks'][TableDataName]['Stage'] % LocationDict[project]['DbLocation'], StartObsTwStr, EndTwStr).getData()
                        PeakObsStage = max(StageGageTsc.values)
                        ...
                        elif PeakFcstStage <= PeakObsStage : 
...
def table1Footnote( debug,          # Set to True to print all debug statements
    ...
    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
    Cell = createCell(debug, Phrase(Chunk('NWS Operational Fcst Including QPF', TableLayoutDict['Table1']['TextFont'])), TableLayoutDict['Table1']['RowSpan'], 
