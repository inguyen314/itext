...
BulletinFilename = "D:\\Projects\\HEC_DSSVue_Scripts\\MRBWM\\NWO-CWMS2_Scripts\\Bulletins\\NWO_Levee\\NWO_Levee_Bulletin.pdf"
CsvFilename = "D:\\Projects\\HEC_DSSVue_Scripts\\MRBWM\\NWO-CWMS2_Scripts\\Bulletins\\NWO_Levee\\NWO_Levee_Bulletin.csv"
BulletinTsFilePathname = "D:\\Projects\\HEC_DSSVue_Scripts\\MRBWM\\NWO-CWMS2_Scripts\\Bulletins\\Bulletins_Time_Series_20171231.txt"
BulletinPropertiesPathname = "D:\\Projects\\HEC_DSSVue_Scripts\\MRBWM\\NWO-CWMS2_Scripts\\Bulletins\\NWO_Levee\\NWO_Levee_Bulletin_Properties_20180626.txt"
...
BulletinFilename = HomeDirectory + 'NWO_Reservoir_Bulletin.pdf'
CsvFilename = HomeDirectory + 'NWO_Reservoir_Bulletin.csv'
BulletinTsFilePathname = HomeDirectory + 'Bulletins_Time_Series_20171231.txt'
BulletinPropertiesPathname = BulletinScriptDirectory + 'NWO_Reservoir_Bulletin_Properties_20180131.txt'
'''
...
def table1Data( debug,      # Set to True to print all debug statements
    ...
    for project in DataBlockDict['DataBlocks'][TableDataName]['ProjectList'] :
        ...
        FloodStage = retrieveLocationLevel(debug, conn, CwmsDb, StageNwsFlood % project)
        ...
        for data in GageData :
            ...
            if data == 'PublicName' :
                ...
                CellPadding = [2, 2, 2, 3]                
            # River Mile of Gage
            elif data == 'River Mile' :
                try :
                    RiverMile = LocationDict[project]['River Mile']
            ...
            # Stage of Gage
                ...
                try :
                    StageGageTsc = CwmsDb.read(DataBlockDict['DataBlocks'][TableDataName][data] % LocationDict[project]['DbLocation']).getData()
                ...
                # Change default cell properties
            # 1-hr Change
            elif data == '1-Hr Change' :
                try :
                    StageGageTsc = CwmsDb.read(DataBlockDict['DataBlocks'][TableDataName]['Stage'] % LocationDict[project]['DbLocation']).getData()
                    if StageGage == Missing or StageGageTsc.values[-2] == Constants.UNDEFINED : raise ValueError
                    Stage1HrChange = StageGageTsc.values[-1] - StageGageTsc.values[-2]
            ...
            # Peak Forecasted Stage
                ...
                try :
                    if project == 'BVNE' :
                        ...
                        FcstStageTsc = CwmsDb.read(DataBlockDict['DataBlocks'][TableDataName][data] % LocationDict[project]['DbLocation'], EndTwStr, EndFcstTwStr).getData()
                        PeakFcstStage = max(FcstStageTsc.values)
                        if PeakFcstStage == Constants.UNDEFINED : raise ValueError
            ...
            # Date of Peak Forecasted Stage
                ...
                try :
                    if project == 'BVNE' :
                        ...
                        FcstStageTsc = CwmsDb.get(DataBlockDict['DataBlocks'][TableDataName]['FcstStage'] % LocationDict[project]['DbLocation'], EndTwStr, EndFcstTwStr)
                        PeakFcstStage = max(FcstStageTsc.values)
                        ...
                        DateOfPeakStageStr = DateOfPeakStageHecTime.date(4)
                        DateOfPeakStageStr = DateOfPeakStageStr[ : 5]
            ...
            if data == GageData[-1] :
                ...
                ColSpan = Table.getNumberOfColumns() - TotalColSpan
        ...
        for levee in LeveeList :
            ...
            ToeOfLevee = FloodStage

            # Add a blank cell to fill in river gage data cells
            CellData = Phrase(Chunk(' ', TextFont))
            # Change default cell properties
            # Default cell properties. If there is a special case, the properties will be changed.
            TextFont = TableLayoutDict[TableName]['TextFont']
            RowSpan = TableLayoutDict[TableName]['RowSpan']; ColSpan = len(GageData)
            HorizontalAlignment = TableLayoutDict[TableName]['HorizontalAlignment']; VerticalAlignment = TableLayoutDict[TableName]['VerticalAlignment']
            CellPadding = TableLayoutDict[TableName]['CellPadding']
            BorderColors = [Color2, Color3, Color2, Color2]
            if LastLine : BorderWidths = [0.25, 0.5, 1, 1]
            ...
            # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
            Cell = createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
            if levee == LeveeList[0]  and len(LeveeList) > 1 : 
                outputDebug(debug, lineNo(), 'Set no bottom border for levee')
                Cell.setBorder(Rectangle.TOP | Rectangle.RIGHT | Rectangle.LEFT)
            elif levee == LeveeList[-1] and len(LeveeList) > 1 : 
                outputDebug(debug, lineNo(), 'Set no top border for levee')
                Cell.setBorder(Rectangle.BOTTOM | Rectangle.RIGHT | Rectangle.LEFT)
            elif len(LeveeList) == 1 :
                outputDebug(debug, lineNo(), 'Do not change borders')
                pass
            else : 
                outputDebug(debug, lineNo(), 'Set no top or bottom border for levee')
            ...
            for data in LeveeData :
                ...
                # River Mile of Levee
                elif data == 'LeveeRiverMile' :
                    try :
                        LeveeRiverMile = Missing
                        
                        # Create a formatted string that will be added to the table
                        CellData = Phrase(Chunk(LeveeRiverMile, TextFont))
                    except :
                        LeveeRiverMile = Missing
                        # Create a formatted string that will be added to the table
                        CellData = Phrase(Chunk(Missing, TextFont))
                ...
                # Percent Loading
                    ...
                    try :
                        ...
                        # Change background color depending on PercentLoading
                        if PercentLoading <= DataBlockDict['DataBlocks'][TableDataName]['GreenFreeboard'] : BackgroundColor = Color8 # Green
                        elif DataBlockDict['DataBlocks'][TableDataName]['GreenFreeboard'] < PercentLoading <= DataBlockDict['DataBlocks'][TableDataName]['YellowFreeboard'] : BackgroundColor = Color9 # Yellow
                        elif DataBlockDict['DataBlocks'][TableDataName]['YellowFreeboard'] < PercentLoading <= DataBlockDict['DataBlocks'][TableDataName]['OrangeFreeboard'] : BackgroundColor = Color11 # Orange
                        elif DataBlockDict['DataBlocks'][TableDataName]['OrangeFreeboard'] < PercentLoading : BackgroundColor = Color10 # Red
                    except :
                 ...
                 # Fcst Percent Loading of Levee
                    ...
                    try :
                        if project == 'BVNE' :
                            ...
                            # Change background color depending on PercentLoading
                            if FcstPercentLoading <= DataBlockDict['DataBlocks'][TableDataName]['GreenFreeboard'] : BackgroundColor = Color8 # Green
                            elif DataBlockDict['DataBlocks'][TableDataName]['GreenFreeboard'] < FcstPercentLoading <= DataBlockDict['DataBlocks'][TableDataName]['YellowFreeboard'] : BackgroundColor = Color9 # Yellow
                            elif DataBlockDict['DataBlocks'][TableDataName]['YellowFreeboard'] < FcstPercentLoading <= DataBlockDict['DataBlocks'][TableDataName]['OrangeFreeboard'] : BackgroundColor = Color11 # Orange
                            elif DataBlockDict['DataBlocks'][TableDataName]['OrangeFreeboard'] < FcstPercentLoading : BackgroundColor = Color10 # Red
                    except :
...
def table1Footnote( debug,          # Set to True to print all debug statements
    ...
    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
    Cell = createCell(debug, Phrase(Chunk('Fcst based on NWS 0 days of QPF', TableLayoutDict['Table1']['TextFont'])), TableLayoutDict['Table1']['RowSpan'], 
        Table1Columns - 4, Element.ALIGN_LEFT, TableLayoutDict['Table1']['VerticalAlignment'], TableLayoutDict['Table1']['CellPadding'], TableLayoutDict['Table1']['BorderColors'], 
        TableLayoutDict['Table1']['BorderWidths'], TableLayoutDict['Table1']['VariableBorders'], TableLayoutDict['Table1']['BackgroundColor'])
    Cell.setBorder(Rectangle.NO_BORDER)
    TableFootnote.addCell(Cell)
...
def table1Heading(  debug,  # Set to True to print all debug statements
    ...
    # Column 3 Heading
    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
    CellData = Phrase(Chunk('1-Hr Change\n(feet)', TableLayoutDict['Table1']['TextFont']))
    Cell = createCell(debug, CellData, TableLayoutDict['Table1']['RowSpan'], TableLayoutDict['Table1']['ColSpan'], 
        Element.ALIGN_CENTER, Element.ALIGN_BOTTOM, TableLayoutDict['Table1']['CellPadding'], TableLayoutDict['Table1']['BorderColors'], 
        [1, 0.25, 0.25, 0.25], TableLayoutDict['Table1']['VariableBorders'], Color6)
    Table.addCell(Cell)
    ...
    # Column 5 Heading
    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
    CellData = Phrase(Chunk('Date of\nPeak Stage', TableLayoutDict['Table1']['TextFont']))
    Cell = createCell(debug, CellData, TableLayoutDict['Table1']['RowSpan'], TableLayoutDict['Table1']['ColSpan'], 
...
try :
    ...
    StartTw             = Date - datetime.timedelta(hours = 2)
    ...
    #
    DataBlocks = ['Data1', 'Data2', 'Data3', 'Data4']
