...
BulletinFilename = "D:\\Projects\\HEC_DSSVue_Scripts\\MRBWM\\NWO-CWMS2_Scripts\\cronjobs\\Bulletins\\NWD_Reservoir\\NWD_Reservoir_Bulletin.pdf"
ArchiveBulletinFilename = "D:\\Projects\\HEC_DSSVue_Scripts\\MRBWM\\NWO-CWMS2_Scripts\\cronjobs\\Bulletins\\NWD_Reservoir\\MRBWM_Reservoir_Bulletin_%s.pdf"
CsvFilename = "D:\\Projects\\HEC_DSSVue_Scripts\\MRBWM\\NWO-CWMS2_Scripts\\cronjobs\\Bulletins\\NWD_Reservoir\\NWD_Reservoir_Bulletin.csv"
CronjobsDirectory = "D:\\Projects\\HEC_DSSVue_Scripts\\MRBWM\\NWO-CWMS2_Scripts\\cronjobs"
BulletinPropertiesPathname = "D:\\Projects\\HEC_DSSVue_Scripts\\MRBWM\\NWO-CWMS2_Scripts\\cronjobs\\Bulletins\\NWD_Reservoir\\NWD_Reservoir_Bulletin_Properties.txt"
'''
                ...
                try :
                    if project in ['SYS'] :
                        ...
                        if project in ['BONY', 'TREN', 'ENDS', 'REWI', 'MECR', 'NORN', 'HACO', 'LOVL', 'MILD', 
                            ...
                            try : Tsc = CwmsDb.read(DataBlockDict['DataBlocks'][TableDataName][data] % project, MkcStartTwStr, MkcEndTwStr).getData()
                            except : Tsc = CwmsDb.read(DataBlockDict['DataBlocks'][TableDataName][data] % LocationDict[project]['DbLocation'], MkcStartTwStr, MkcEndTwStr).getData()
                        # These NWk projects have 2400 values but use GMT-6 to retrieve the data
                        elif project in ['STON', 'HAST', 'BAGL'] :
                            CwmsDb.setTimeZone('Etc/GMT+6')
                            try : Tsc = CwmsDb.read(DataBlockDict['DataBlocks'][TableDataName][data] % project).getData()
                            except : Tsc = CwmsDb.read(DataBlockDict['DataBlocks'][TableDataName][data] % LocationDict[project]['DbLocation']).getData()
                        # Once all time series have been renamed, this block can be the else block that uses the project as its base location
                        try : Tsc = CwmsDb.read(DataBlockDict['DataBlocks'][TableDataName][data] % project).getData()
                        except : Tsc = CwmsDb.read(DataBlockDict['DataBlocks'][TableDataName][data] % LocationDict[project]['DbLocation']).getData()
                ...
                try :
                    if project in ['SYS'] :
                        ...
                        if project in ['BONY', 'TREN', 'ENDS', 'REWI', 'MECR', 'NORN', 'HACO', 'LOVL', 'MILD', 'CEBL', 'KANS', 'WILN', 'KIRN', 'WEBR', 
                                       'GLEL', 'TUCR', 'PERY', 'CLIN', 'BSPM', 'LGVM', 'SMIE', 'RATN', 'LNGB', 'POMA', 'MELN', 'HILS', 'PODT', 'STON', 'HAST', 'BAGL'] :
                        ...
                        if project not in ['FTPK', 'GARR', 'OAHE', 'BEND', 'FTRA', 'GAPT'] :
                            ...
                            if project in ['BONY', 'TREN', 'ENDS', 'REWI', 'MECR', 'NORN', 'HACO', 'LOVL', 'MILD', 
                                           ...
                                           'BSPM', 'LGVM', 'SMIE', 'RATN', 'LNGB', 'POMA', 'MELN', 'HILS', 'PODT'] :
                                CwmsDb.setTimeZone('Etc/GMT+6')
                                try : Tsc = CwmsDb.read(DataBlockDict['DataBlocks'][TableDataName][data] % project, MkcTrimTwStr, MkcEndTwStr).getData()
                                except : Tsc = CwmsDb.read(DataBlockDict['DataBlocks'][TableDataName][data] % LocationDict[project]['DbLocation'], MkcTrimTwStr, MkcEndTwStr).getData()
                            else :
                                CwmsDb.setTimeZone('Etc/GMT+6')
