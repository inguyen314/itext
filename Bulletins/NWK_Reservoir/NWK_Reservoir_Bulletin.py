...
import java.lang
import os, sys, inspect, datetime, time, DBAPI, traceback, java
...
if OsName[ : 7] == 'windows' : 
    ...
    ScriptDirectory = BulletinsDirectory + 'NWK_Reservoir\\'
    BulletinFilename = BulletinsDirectory + 'NWK_Reservoir_Bulletin.pdf'
    CsvFilename = BulletinsDirectory + 'NWK_Reservoir_Bulletin.csv'
    BulletinPropertiesPathname = ScriptDirectory + 'NWK_Reservoir_Bulletin_Properties.txt'
else :
    # Server pathnames
    ...
    for project in DataBlockDict['DataBlocks'][TableDataName]['ProjectList'] :
        ...
        for data in DataOrder :
            ...
            if data == 'PublicName' :
                ...
                if project == 'BAGL' : BulletinName = BulletinName + '*' 
            ...
            # Elevation
                ...
                try :
                    ...
                    # Use daily elevation time series for these projects
                    if project in ['BAGL', 'BONY', 'TREN', 'ENDS', 'REWI', 'MECR', 'NORN', 'CEBL'] :
                        if ElevInstDayBestNwk % project in PathnameList :
                            outputDebug(debug, lineNo(), 'Pathname = ', ElevInstDayBestNwk % project)
                            Tsc = CwmsDb.read(ElevInstDayBestNwk % project).getData()
                        else :
                            raise ValueError
                    else :  
                        if DataBlockDict['DataBlocks'][TableDataName][data] % project in PathnameList :
                            outputDebug(debug, lineNo(), 'Pathname = ', DataBlockDict['DataBlocks'][TableDataName][data] % project)
...
def table1Footnote( debug,          # Set to True to print all debug statements
    ...
    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
    Cell = createCell(debug, Phrase(Chunk('Data for Stockton, Harry S Truman, and Bagnell are 0000 values.', 
