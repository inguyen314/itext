...
if OsName[ : 7] == 'windows' : 
    ...
    BulletinFilename = BulletinsDirectory + 'MVS_Daily_River_Bulletin.pdf'
...
import Server_Utils
reload(Server_Utils)
from Server_Utils import lineNo, outputDebug, retrieveLocationLevel, retrievePublicName, retrieveElevatonDatum, retrieveRiverMile, retrieveGroup, retrieveGroupLPMS, retrieveGageZero29, retrieveBasin, retrieveLocationID
...
def table1Data( debug,      # Set to True to print all debug statements
    ...
    getBasin = retrieveBasin(debug,conn,) 
    for basin in getBasin:
        print "basin: " + str(basin)
        getLocationID = retrieveLocationID(debug,conn,basin,) 
        #for project in getLocationID:
        ...
        for data in DataOrder :
            ...
            # 4 - Elevation
                ...
                try :
                    ...
                    #if PrevElev >= NwsFloodStage: BackgroundColor = Color10 # Red
                    ...
                    if project in GroupSet:
                        ...
                        print 'NwsFloodStage = ' + str(NwsFloodStage)
                        print 'PrevElev = ' + str(PrevElev)
                        print 'PrevElev + getGageZero29 = ' + str(PrevElev + float(getGageZero29))
                        if str(NwsFloodStage)!='--':
                            if (float(PrevElev) >= (float(NwsFloodStage) + float(getGageZero29))): BackgroundColor = Color10 # Red            
                    elif str(NwsFloodStage) == '--': 
                        BackgroundColor = Color4
                        print "colorset = Color4" 
                    else:
                        if (float(PrevElev) >= float(NwsFloodStage)): 
...
try :    
    ...
    if is_dst(str(Date)):
       ...
       StartTribTwStr      = EndTw.strftime('%d%b%Y 0600')
       EndTribStr          = EndTribTwStr.strftime('%d%b%Y 0600') 
    else:
    ...
    #Select Tribs
    #TrimTwStr2          = EndTw.strftime('%d%b%Y 0600') 
    #EndTwStr2           = Date.strftime('%d%b%Y 0600')
    #EndTribTw           = Date - datetime.timedelta(2)
    #TribTrimTwStr       = EndTribTw.strftime('%d%b%Y 0600')
    #TribEndTwStr        = EndTribTw.strftime('%d%b%Y 0600') 
