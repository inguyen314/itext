...
if OsName[ : 7] == 'windows' : 
    # PC pathnames
    CronjobsDirectory = "C:\\Users\\G0PDRRJL\\Documents\\Projects\\Scripts\\MRBWM\\NWO-CWMS2_Scripts\\g7cwmspd\\cronjobs\\" # Used in the properties file to create pathname for Seals and Symbols
    BulletinsDirectory = CronjobsDirectory + 'Bulletins\\'
    ScriptDirectory = BulletinsDirectory + 'NWO_Reservoir\\'
    BulletinFilename = BulletinsDirectory + 'NWO_Reservoir_Bulletin.pdf'
    CsvFilename = BulletinsDirectory + 'NWO_Reservoir_Bulletin.csv'
    BulletinPropertiesPathname = ScriptDirectory + 'NWO_Reservoir_Bulletin_Properties.txt'
else :
    # Server pathnames
...
def table1Footnote( debug,          # Set to True to print all debug statements
    ...
    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
    Cell = createCell(debug, Phrase(Chunk('All USBR tributary project elevations are reported in NGVD29. All Corps tributary project elevations are reported in local project datums.', 
