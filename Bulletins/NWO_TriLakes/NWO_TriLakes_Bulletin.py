...
Description: Create the Omaha District Tri-Lakes Bulletin
'''
...
import java.lang
import os, sys, inspect, datetime, time, DBAPI, java, traceback
...
if OsName[ : 7] == 'windows' : 
    # PC pathnames
    CronjobsDirectory = r"C:\Users\G0PDRRJL\Documents\Projects\Scripts\MRBWM\NWO-CWMS2_Scripts\g7cwmspd\cronjobs\\" # Used in the properties file to create pathname for Seals and Symbols
    BulletinsDirectory = CronjobsDirectory + 'Bulletins\\'
    ScriptDirectory = BulletinsDirectory + 'NWO_TriLakes\\'
    BulletinFilename = BulletinsDirectory + 'NWO_TriLakes_Bulletin.pdf'
    CsvFilename = BulletinsDirectory + 'NWO_TriLakes_Bulletin.csv'
    BulletinPropertiesPathname = ScriptDirectory + 'NWO_TriLakes_Bulletin_Properties.txt'
else :
    # Server pathnames
...
def table2Data( debug,      # Set to True to print all debug statements
    ...
    for project in DataBlockDict['DataBlocks'][TableDataName]['ProjectList'] :
        ...
        # If adding the last project in the last data block, create a trigger to use a thick bottom border
        if DataName == DataBlocks[-1] and project == 'KECL' :
            LastProject = True
        else : LastProject = False
...
def table3Data( debug,      # Set to True to print all debug statements
    ...
    Img.scalePercent(20)
    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
    Cell = createCell(debug, Img, 44, TableLayoutDict['Table3']['ColSpan'], Element.ALIGN_LEFT, 
    ...
    # Add blank column between image & smaller tables
    # Column 2 Heading
    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
    CellData = Phrase(Chunk(' ', TableLayoutDict['Table3']['TextFont']))
    Cell = createCell(debug, CellData, 44, TableLayoutDict['Table3']['ColSpan'], Element.ALIGN_CENTER, 
        TableLayoutDict['Table3']['VerticalAlignment'], TableLayoutDict['Table3']['CellPadding'], [Color2, Color3, Color2, Color2], 
        [0.25, 0.5, 0.25, 1], TableLayoutDict['Table3']['VariableBorders'], Color4)
    Cell.setBorder(Rectangle.NO_BORDER)
    Table.addCell(Cell)
    ...
    for project in DataBlockDict['DataBlocks'][TableDataName]['ProjectList'] :
        ...
        # Retrieve Public Name and store it to the DataBlockDict
        SmallTableHeader = retrievePublicName(debug, conn, project)
        BulletinName = SmallTableHeader.replace(' & Reservoir', '')
        outputDebug(debug, lineNo(), 'Creating %s row' % BulletinName)
        
        # Add Table3 Headers (i.e., Bear Creek Dam \nDaily Maximums of Record)
        # Column 3-5 Heading
        # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
        CellData = Phrase(Chunk(BulletinName + '\nDaily Maximums of Record', Font3))
        Cell = createCell(debug, CellData, 2, 3, Element.ALIGN_CENTER, 
            TableLayoutDict['Table3']['VerticalAlignment'], TableLayoutDict['Table3']['CellPadding'], [Color2, Color2, Color2, Color2], 
            [1, 1, 0.5, 1], TableLayoutDict['Table3']['VariableBorders'], Color5)
        Table.addCell(Cell)
        # Add data to CsvData.
        UnformattedData = str(CellData[0]).replace(',', '') + ','
        CsvData += UnformattedData
        
        # Set statistic types that will be calculated
        for y in ['Inflow', 'Outflow', 'Elevation'] :
        
            # Set variable x = 0 to use as an index for arrays in Table3 properties
            x = 0
            
            while x < 3 :
                outputDebug(True, lineNo(), 'At start of while loop x = ', x)

                # Only generate sub-headings if at the first index of the array
                if x == 0 :
                
                    # Column 3 Blank Sub-heading
                    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
                    CellData = Phrase(Chunk(' ', TableLayoutDict['Table3']['TextFont']))
                    Cell = createCell(debug, CellData, TableLayoutDict['Table3']['RowSpan'], TableLayoutDict['Table3']['ColSpan'], Element.ALIGN_CENTER,
                        TableLayoutDict['Table3']['VerticalAlignment'], TableLayoutDict['Table3']['CellPadding'], [Color2, Color2, Color2, Color2], 
                        [0.5, 0.25, 0.5, 1], TableLayoutDict['Table3']['VariableBorders'], Color6)
                    Table.addCell(Cell)
                    # Add data to CsvData.
                    UnformattedData = str(CellData[0]).replace(',', '') + ','
                    CsvData += UnformattedData
                    
                    # Column 4 Parameter Sub-heading
                    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
                    CellData = Phrase(Chunk(y, TableLayoutDict['Table3']['TextFont']))
                    Cell = createCell(debug, CellData, TableLayoutDict['Table3']['RowSpan'], TableLayoutDict['Table3']['ColSpan'], Element.ALIGN_CENTER,
                        TableLayoutDict['Table3']['VerticalAlignment'], TableLayoutDict['Table3']['CellPadding'], [Color2, Color2, Color2, Color2], 
                        [0.5, 0.25, 0.5, 0.25], TableLayoutDict['Table3']['VariableBorders'], Color6)
                    Table.addCell(Cell)
                    # Add data to CsvData.
                    UnformattedData = str(CellData[0]).replace(',', '') + ','
                    CsvData += UnformattedData
                    
                    # Column 5 Date Sub-heading
                    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
                    CellData = Phrase(Chunk('Date', TableLayoutDict['Table3']['TextFont']))
                    Cell = createCell(debug, CellData, TableLayoutDict['Table3']['RowSpan'], TableLayoutDict['Table3']['ColSpan'], Element.ALIGN_CENTER,
                        TableLayoutDict['Table3']['VerticalAlignment'], TableLayoutDict['Table3']['CellPadding'], [Color2, Color2, Color2, Color2], 
                        [0.5, 1, 0.5, 0.25], TableLayoutDict['Table3']['VariableBorders'], Color6)
                    Table.addCell(Cell)
                    # Add data to CsvData.
                    UnformattedData = str(CellData[0]).replace(',', '') + ','
                    CsvData += UnformattedData
            
                outputDebug(True, lineNo(), 'DataOrder3 = ', DataOrder3)
                # Reset TotalColSpan
                TotalColSpan = 2
                for data in DataOrder3 :
                
                    outputDebug(True, lineNo(), 'Adding %s to the row' % data, ' x = ', x)
                    # Create a variable within the DataDict. This will allow the user to store all data to a dictionary and access the variables throughout
                    #   the script
                    DataBlockDict['DataBlocks'][TableDataName].setdefault(project, {}).setdefault(data, None)

                    # Get column number
                    ColumnKey = 'Column%d' % DataOrder3.index(data)
                    ...
                    if data == 'Ranks' :
                        try :
                            Rank = DataBlockDict['DataBlocks'][TableDataName][data][x]
                            
                            outputDebug(debug, lineNo(), '%s Ranking = ' % project, Rank)
                            
                            # Create a formatted string that will be added to the table
                            CellData = Phrase(Chunk(Rank, TextFont))
                        ...
                        # Store value to DataDict
                        DataBlockDict['DataBlocks'][TableDataName][project][data] = Rank

                        # Change default cell properties
                        HorizontalAlignment = Element.ALIGN_LEFT
                        if x == 0 :
                            BorderWidths = [0.5, 0.25, 0.25, 1]
                            BorderColors = [Color3, Color2, Color2, Color2]
                        elif x == 1 :
                            BorderWidths = [0.25, 0.25, 0.25, 1]
                        elif x == 2 :
                            if y == 'Elevation' :
                                BorderWidths = [0.25, 0.25, 1, 1]
                            else :
                                BorderWidths = [0.25, 0.25, 0.5, 1]
                                BorderColors = [Color2, Color2, Color3, Color2]
                        CellPadding = [2, 2, 2, 3]                

                    # Statistical Values
                    elif data == 'Values' :
                        outputDebug(True, lineNo(), 'Start of Values x = ', x)
                        try :
                            Label = 'DEF'
                            '''
                            Tsc = DataBlockDict['DataBlocks'][TableDataName][data][x] % project
                            
                            # Reset time window to cover entire POR of data to calculate water year statistics
                            if project == 'BECR' : StartTwStr = '01May1979 2400'
                            elif project == 'CHCR' : StartTwStr = '01Mar1960 2400'
                            elif project == 'CHFI' : StartTwStr = '01Jun1979 2400'
                            outputDebug(debug, lineNo(), 'Start of Time Window for Project Statistics = ', StartTwStr)
                        
                            # Set data dictionary defaults to store statistics
                        
                        
                            # Reset starting time window for rest of bulletin
                        ...
                        # Change default cell properties
                        if x == 0 :
                            BorderWidths = [0.5, 0.25, 0.25, 0.25]
                            BorderColors = [Color3, Color2, Color2, Color2]
                        elif x == 2 :
                            if y == 'Elevation' :
                                BorderWidths = [0.25, 0.25, 1, 0.25]
                            else :
                                BorderWidths = [0.25, 0.25, 0.5, 0.25]
                                BorderColors = [Color2, Color2, Color3, Color2]
                        CellPadding = [2, 2, 2, 3]
                        outputDebug(True, lineNo(), 'End of Values x = ', x)
                        
                    # Dates associated with statistical values
                    elif data == 'Dates' :
                        outputDebug(True, lineNo(), 'Dates x = ', x)
                        try :
                            Date = 'ABC'

                            # Create a formatted string that will be added to the table
                            CellData = Phrase(Chunk(Date, TextFont))
                        ...
                        # Store value to DataDict
                        DataBlockDict['DataBlocks'][TableDataName][project][data] = Date
                    
                        # Change default cell properties
                        if x == 0 :
                            BorderWidths = [0.5, 1, 0.25, 0.25]
                            BorderColors = [Color3, Color2, Color2, Color2]
                        elif x == 1 :
                            BorderWidths = [0.25, 1, 0.25, 0.25]
                        elif x == 2 :
                            if y == 'Elevation' :
                                BorderWidths = [0.25, 1, 1, 0.25]
                            else :
                                BorderWidths = [0.25, 1, 0.5, 0.25]
                                BorderColors = [Color2, Color2, Color3, Color2]
                    ...
                    CsvData += UnformattedData
                    CsvData += ','
                    outputDebug(True, lineNo(), 'ColSpan = ', ColSpan, '  TotalColSpan = ', TotalColSpan, '  Total Table Columns = ', Table.getNumberOfColumns())
                    if TotalColSpan == Table.getNumberOfColumns() :
                        CsvData += '\n'
                        break
                    
                # Increase variable x to use as an index for arrays in Table3 properties
                x = x + 1
                
        if project == 'BECR' or project == 'CHFI' :
            
            # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
            Cell = createCell(debug,  Phrase(Chunk('', Font1)), TableLayoutDict['Table3']['RowSpan'], 3, Element.ALIGN_CENTER, 
                TableLayoutDict['Table3']['VerticalAlignment'], TableLayoutDict['Table3']['CellPadding'], [Color2, Color2, Color2, Color2], 
                [1, 1, 0.5, 1], TableLayoutDict['Table3']['VariableBorders'], TableLayoutDict['Table3']['BackgroundColor'])
            Cell.setBorder(Rectangle.NO_BORDER)
            Table.addCell(Cell)
...
def table1Footnote( debug,          # Set to True to print all debug statements
    ...
    Cell = createCell(debug, Phrase(Chunk('All project elevations are reported in local project datums.', 
        TableLayoutDict['Table1']['TextFont'])), TableLayoutDict['Table1']['RowSpan'], 5, Element.ALIGN_LEFT, TableLayoutDict['Table1']['VerticalAlignment'], 
        TableLayoutDict['Table1']['CellPadding'], TableLayoutDict['Table1']['BorderColors'], TableLayoutDict['Table1']['BorderWidths'], 
        TableLayoutDict['Table1']['VariableBorders'], TableLayoutDict['Table1']['BackgroundColor'])
    Cell.setBorder(Rectangle.NO_BORDER)
    TableFootnote.addCell(Cell)
...
def table2Footnote( debug,          # Set to True to print all debug statements
    ...
    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
    Cell = createCell(debug, Phrase(Chunk('US = Upstream of', TableLayoutDict['Table2']['TextFont'])), TableLayoutDict['Table2']['RowSpan'], 
    ...
    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
    Cell = createCell(debug, Phrase(Chunk('DS = Downstream of',  TableLayoutDict['Table2']['TextFont'])), TableLayoutDict['Table2']['RowSpan'], 
        2, Element.ALIGN_LEFT, TableLayoutDict['Table2']['VerticalAlignment'], TableLayoutDict['Table2']['CellPadding'], TableLayoutDict['Table2']['BorderColors'], 
        TableLayoutDict['Table2']['BorderWidths'], TableLayoutDict['Table2']['VariableBorders'], TableLayoutDict['Table2']['BackgroundColor'])
    Cell.setBorder(Rectangle.NO_BORDER)
    TableFootnote.addCell(Cell)
    ...
    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
    Cell = createCell(debug, Phrase(Chunk('BECR = Bear Creek Dam',  TableLayoutDict['Table2']['TextFont'])), TableLayoutDict['Table2']['RowSpan'], 
        2, Element.ALIGN_LEFT, TableLayoutDict['Table2']['VerticalAlignment'], TableLayoutDict['Table2']['CellPadding'], TableLayoutDict['Table2']['BorderColors'], 
        TableLayoutDict['Table2']['BorderWidths'], TableLayoutDict['Table2']['VariableBorders'], TableLayoutDict['Table2']['BackgroundColor'])
    Cell.setBorder(Rectangle.NO_BORDER)
    TableFootnote.addCell(Cell)
    ...
    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
    Cell = createCell(debug, Phrase(Chunk('CHFI = Chatfield Dam',  TableLayoutDict['Table2']['TextFont'])), TableLayoutDict['Table2']['RowSpan'], 
        2, Element.ALIGN_LEFT, TableLayoutDict['Table2']['VerticalAlignment'], TableLayoutDict['Table2']['CellPadding'], TableLayoutDict['Table2']['BorderColors'], 
        TableLayoutDict['Table2']['BorderWidths'], TableLayoutDict['Table2']['VariableBorders'], TableLayoutDict['Table2']['BackgroundColor'])
    Cell.setBorder(Rectangle.NO_BORDER)
    TableFootnote.addCell(Cell)
    ...
    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
    Cell = createCell(debug, Phrase(Chunk('CHCR = Cherry Creek Dam',  TableLayoutDict['Table2']['TextFont'])), TableLayoutDict['Table2']['RowSpan'], 
        2, Element.ALIGN_LEFT, TableLayoutDict['Table2']['VerticalAlignment'], TableLayoutDict['Table2']['CellPadding'], TableLayoutDict['Table2']['BorderColors'], 
        TableLayoutDict['Table2']['BorderWidths'], TableLayoutDict['Table2']['VariableBorders'], TableLayoutDict['Table2']['BackgroundColor'])
    Cell.setBorder(Rectangle.NO_BORDER)
    TableFootnote.addCell(Cell)
    ...
    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
    Cell = createCell(debug, Phrase(Chunk('* Stage value is above the Maximum of Record.',  TableLayoutDict['Table2']['TextFont'])), TableLayoutDict['Table2']['RowSpan'], 
...
#    # Add a blank line to the table footer to separate the footer from Table2
#    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
#    Cell = createCell(debug, Phrase(Chunk(' ', Font1)),  TableLayoutDict['Table2']['TextFont'])), TableLayoutDict['Table2']['RowSpan'], 
#        12, Element.ALIGN_LEFT, TableLayoutDict['Table2']['VerticalAlignment'], TableLayoutDict['Table2']['CellPadding'], TableLayoutDict['Table2']['BorderColors'], 
#        TableLayoutDict['Table2']['BorderWidths'], TableLayoutDict['Table2']['VariableBorders'], TableLayoutDict['Table2']['BackgroundColor'])
#    Cell.setBorder(Rectangle.NO_BORDER)
...
def table2Heading(  debug,  # Set to True to print all debug statements
    ...
    # Column 10-11 Heading
    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
#    CellData = Phrase(Chunk('Max of Record', Font3))
...
#    CsvData += UnformattedData
    ...
    Cell = createCell(debug, CellData, TableLayoutDict['Table2']['RowSpan'], TableLayoutDict['Table2']['ColSpan'], 
        ...
        [Color2, Color2, Color2, Color2], [0.25, 0.25, 0.25, 0.25], TableLayoutDict['Table2']['VariableBorders'], 
        Color6)
    Cell.setFixedHeight(24)
    Table.addCell(Cell)
    # Add data to CsvData.
    ...
    # Column 10 Sub-Heading
    # createCell(debug, CellData, RowSpan, ColSpan, HorizontalAlignment, VerticalAlignment, CellPadding, BorderColors, BorderWidths, VariableBorders, BackgroundColor)
#    CellData = Phrase(Chunk('Date', TableLayoutDict['Table2']['TextFont']))
#    Cell = createCell(debug, CellData, TableLayoutDict['Table2']['RowSpan'], TableLayoutDict['Table2']['ColSpan'], 
#        Element.ALIGN_CENTER, Element.ALIGN_BOTTOM, TableLayoutDict['Table2']['CellPadding'], [Color2, Color2, Color2, Color3], 
#        [0.25, 0.25, 0.25, 0.5], TableLayoutDict['Table2']['VariableBorders'], Color6)
#    Cell.setFixedHeight(24)
...
#    Cell = createCell(debug, CellData, TableLayoutDict['Table2']['RowSpan'], TableLayoutDict['Table2']['ColSpan'], 
#        Element.ALIGN_CENTER, Element.ALIGN_BOTTOM, TableLayoutDict['Table2']['CellPadding'], [Color2, Color2, Color2, Color2], 
#        [0.25, 1, 0.25, 0.25], TableLayoutDict['Table2']['VariableBorders'], Color6)
#    Cell.setFixedHeight(24)
#    Table.addCell(Cell)
#    # Add data to CsvData.
#    UnformattedData = str(CellData[0]).replace(',', '') + ','
